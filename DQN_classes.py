...
class DQNLearningAgent(object):
    def __init__(self, n_state, n_action, n_hidden=64, lr=1e-4, gamma=0.99,
        ...
        self.gamma = gamma
        # dqn network
        self.q_local = DQN(n_state, n_action, n_hidden).to(device)
        self.q_target = DQN(n_state, n_action, n_hidden).to(device)
        self.optimizer = torch.optim.Adam(
            self.q_local.parameters(), lr=lr)  # weight_decay=0
